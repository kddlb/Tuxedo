...
- [x] HLS (HTTP Live Streaming) decoder (`Cog/Plugins/HLS/*`; routes `.m3u8` + HLS MIME types to a playlist-aware decoder that fetches segments in the background and feeds them to FFmpeg via an in-memory source — uses `src/plugin/input/http_source.*` for fetching and `src/plugin/input/ffmpeg_decoder.*` for the underlying audio decode)
